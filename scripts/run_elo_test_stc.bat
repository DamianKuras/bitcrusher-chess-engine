...
echo  Starting Automated STC SPRT Elo Testing
...
echo Compilations successful. Starting STC fastchess match...
cd /d "%TMP_DIR%"

fastchess -engine cmd="%DEV_DIR%\bin\Release\Uci.exe" name="Bitcrusher_%BRANCH_DEV%" -engine cmd="%BASE_DIR%\bin\Release\Uci.exe" name="Bitcrusher_%BRANCH_BASE%" -each tc=10+0.1 -rounds 500 -games 2 -repeat -concurrency 4 -openings file="%REPO_ROOT%\data\pgn\8move_v3.pgn" format=pgn order=random -sprt elo0=0.0 elo1=5.0 alpha=0.05 beta=0.05

echo.
echo STC Match completed. Cleaning up temporary files...
cd /d "%REPO_ROOT%"
rmdir /s /q "%TMP_DIR%"
echo Done.
