...
consteval std::array<uint64_t, DIAGONAL_COUNT> createDiagonalBitboards() {
    ...
    array[std::to_underlying(Diagonal::A8H1)] =
        convert::toBitboard(Square::A8, Square::B7, Square::C6, Square::D5,
                            Square::E4, Square::F3, Square::G2, Square::H1);
    uint64_t prev_diagonal = array[std::to_underlying(Diagonal::A8H1)];
    for (Diagonal diag :
         {Diagonal::A7G1, Diagonal::A6F1, Diagonal::A5E1, Diagonal::A4D1,
          Diagonal::A3C1, Diagonal::A2B1, Diagonal::A1}) {
        array[std::to_underlying(diag)] = offset::shiftBitboardNoWrap(
            prev_diagonal, offset::calculateOffset(Direction::BOTTOM));
        prev_diagonal = array[std::to_underlying(diag)];
    }
    prev_diagonal = array[std::to_underlying(Diagonal::A8H1)];
    for (auto diag :
         {Diagonal::B8H2, Diagonal::C8H3, Diagonal::D8H4, Diagonal::E8H5,
          Diagonal::F8H6, Diagonal::G8H7, Diagonal::H8}) {
        array[std::to_underlying(diag)] = offset::shiftBitboardNoWrap(
            prev_diagonal, offset::calculateOffset(Direction::RIGHT));
        prev_diagonal = array[std::to_underlying(diag)];
    }
    return array;
}
...
consteval std::array<uint64_t, DIAGONAL_COUNT>
createCounterDiagonalBitboards() {
    std::array<uint64_t, DIAGONAL_COUNT> array{};
    array[std::to_underlying(CounterDiagonal::A1H8)] =
        convert::toBitboard(Square::A1, Square::B2, Square::C3, Square::D4,
    ...
    for (CounterDiagonal counter_diag :
         {CounterDiagonal::A2G8, CounterDiagonal::A3F8, CounterDiagonal::A4E8,
          ...
          CounterDiagonal::A8}) {
        array[std::to_underlying(counter_diag)] = offset::shiftBitboardNoWrap(
            prev_counter_diagonal, offset::calculateOffset(Direction::TOP));
    ...
    for (CounterDiagonal counter_diag :
         {CounterDiagonal::B1H7, CounterDiagonal::C1H6, CounterDiagonal::D1H5,
          ...
          CounterDiagonal::H1}) {
        array[std::to_underlying(counter_diag)] = offset::shiftBitboardNoWrap(
            prev_counter_diagonal, offset::calculateOffset(Direction::RIGHT));
        prev_counter_diagonal = array[std::to_underlying(counter_diag)];
    }
    return array;
}
...
constexpr std::array<uint64_t, DIAGONAL_COUNT>
    PRECOMPUTED_COUNTER_DIAGONAL_BITBOARDS = createCounterDiagonalBitboards();
...
const std::array<uint64_t, SQUARE_COUNT> SQUARE_TO_DIAGONAL_BITBOARD =
    createSquareToDiagonalBitboard();

consteval std::array<uint64_t, SQUARE_COUNT>
createSquareToCounterDiagonalBitboard() {
    ...
    for (Square square = Square::A8; square <= Square::H1; square += 1) {
        ...
        array[std::to_underlying(square)] =
            PRECOMPUTED_COUNTER_DIAGONAL_BITBOARDS[std::to_underlying(
                counter_diagonal)];
    }
    return array;
}
