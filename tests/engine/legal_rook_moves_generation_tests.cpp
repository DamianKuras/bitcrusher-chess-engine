...
#include "legal_move_generators/rook_legal_moves.hpp"
...
namespace {
void generateLegalRookMovesWhite(const BoardState&         board,
                                 const RestrictionContext& restriction_context,
                                 TestMoveSink&             sink) {
    generateLegalRookMoves<Color::WHITE>(board, restriction_context, sink);
}

void generateLegalRookMovesBlack(const BoardState&         board,
                                 const RestrictionContext& restriction_context,
                                 TestMoveSink&             sink) {
    generateLegalRookMoves<Color::BLACK>(board, restriction_context, sink);
}
} // namespace
...
const std::array<LegalMovesTestCase, 7> LEGAL_ROOK_MOVES_TEST_CASES{{
    ...
    {.name = "White rook pinned along file should be able to capture pinning piece along file.",
